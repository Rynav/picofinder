      ...
      <main className="flex min-h-screen flex-col items-center justify-center gap-2 ">
        <form className='flex flex-col text-center gap-2'>
          ...
          <div className='flex flex-row items-center gap-2'>
            <Input type={"text"}  placeholder='Title' value={title} onChange={(e) => setTitle(e.target.value)} name='title' className='w-full' disabled={loading} />
          </div>

          <h1 >Duration</h1>
          <div className='flex flex-row items-center gap-2'>
            <Input type={"text"} placeholder='From' value={lengthFrom} onChange={(e) => setLengthFrom(e.target.value.replace(/[^[0-9]:]/g, ""))} name='dateFrom' className='w-full' error={!lengthFrom.match(/^(\d|1\d|2[0-3]):[0-5]\d$/) && lengthFrom.length > 1} />
            -
            <Input type={"text"} placeholder='To' value={lengthTo} onChange={(e) => setLengthTo(e.target.value.replace(/[^[0-9]:]/g, ""))} className='w-full' name='dateTo input' error={!lengthTo.match(/^(\d|1\d|2[0-3]):[0-5]\d$/) && lengthTo.length > 1} />
          </div>
          <div>
            <h1>Upload date</h1>
            <div className="flex flex-row items-center gap-2">
              <Dropdown value={month} setValue={setMonth} open={monthOpen} setOpen={setMonthOpen} values={months} label={"Month..."}/>
              <Dropdown value={year} setValue={setYear} open={yearOpen} setOpen={setYearOpen} values={years} label={"Year..."}/>
            </div>
          </div>

          <h1>Encoder</h1>
          <div className="flex flex-row items-center gap-2">
            <Dropdown value={encoder} setValue={setEncoder} open={encoderOpen} setOpen={setEncoderOpen} values={encoders} label={"Encoder..."}/>
          </div>
          {encoder == "Custom" && (
              <div>
                <h1>Custom encoder</h1>
                <div className="flex flex-row items-center gap-2">
                  <input type={"text"} placeholder='Custom encoder' value={customEncoder} onChange={(e) => setCustomEncoder(e.target.value)} name='custom encoder' className='w-full input ' disabled={loading} />
              </div>
            </div>
          )}
          
        </form>
        <button className={"btn"} onClick={fetchData} disabled={loading || (!lengthFrom.match(/^(\d|1\d|2[0-3]):[0-5]\d$/) && lengthFrom.length > 1 || !lengthTo.match(/^(\d|1\d|2[0-3]):[0-5]\d$/) && lengthTo.length > 1)}>Submit</button>
